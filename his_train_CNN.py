...
# Will have to change data feed individually, since we are not using a text file
...
tf.app.flags.DEFINE_string("training_data_path", "All_Data/data.txt", "path of training data")
tf.app.flags.DEFINE_integer("vocab_size",len(string.printable),"maximum vocab size.")  # since we are embedding characters
tf.app.flags.DEFINE_string("cache_file_h5py","../All_Data/data.h5","path of training/validation/test data.") #../data/sample_multiple_label.txt
tf.app.flags.DEFINE_string("cache_file_pickle","../All_Data/vocab_label.pik","path of vocabulary and label files") #../data/sample_multiple_label.txt


# Kept most of his default flag data
...
tf.app.flags.DEFINE_integer("sentence_len",100,"max sentence length") # Changed
tf.app.flags.DEFINE_integer("embed_size",100,"embedding size") # Changed from 128
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                feed_dict = {textCNN.X_in: trainX[start:end],textCNN.dropout_keep_prob: 0.8,textCNN.is_training_flag:FLAGS.is_training_flag}
                
                feed_dict[textCNN.y_in] = trainY[start:end]
